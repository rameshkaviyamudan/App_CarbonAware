...
public class GraphFragment1 extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (bundle != null && bundle.containsKey("userStatData")) {
            ...
            // Use these values to update your graph as required

            List<BarEntry> entries = new ArrayList<>();
            entries.add(new BarEntry(0, userTotalCarbonFootprint)); // User
            entries.add(new BarEntry(1, 5300f)); // World
            entries.add(new BarEntry(2, 4600f)); // SEA
            entries.add(new BarEntry(3, 3900f)); // Singapore

            BarDataSet dataSet = new BarDataSet(entries, "Carbon Footprint");
            dataSet.setColors(ColorTemplate.COLORFUL_COLORS);
            // Add title to the BarDataSet
            dataSet.setLabel("Overall Carbon Footprint"); // Set the title for the BarDataSet
